...
def Q_learning(RL_params,physics_params,theta=None,tilings=None,greedy=False):
	...
	for j in xrange(N_episodes):
		...
		for t_step in xrange(max_t_steps): #
			...
			#print 'traces', np.round( np.sum(  e[ RL.find_feature_inds(hx_i,tilings,N_tiles) ,0 ,: ],axis=0), 3)
			"""
		...
		if (j%100==0 and j!=0) or (np.round(fidelity,5) == 1.0):
			...
			F_inst, E_inst, dE_inst, Sent_inst = Fidelity(psi_i,H_fid,t_vals,basis=basis,psi_f=psi_f,all_obs=True)

			t_vals, p_vals = t_greedy, protocol_greedy
			F_greedy, E_greedy, dE_greedy, Sent_greedy = Fidelity(psi_i,H_fid,t_vals,basis=basis,psi_f=psi_f,all_obs=True)

			t_vals, p_vals = t_best, protocol_best
			F_best, E_best, dE_best, Sent_best = Fidelity(psi_i,H_fid,t_vals,basis=basis,psi_f=psi_f,all_obs=True)
