...
class UTILS:
	...
	def print_parameters(self, parameters):
		...
		Parameters
			...
			norm: whether to use fidelity wrt target GS or the entire target basis (GS vs trace norm)
		...
		symmetrize,outfile,norm = tuple([parameters[s] for s in ['symmetrize','outfile','norm']])
		...
		print("cost function norm: \t %s norm"%str(norm))
		...
		if os.path.isfile(file_name):
			try:
				with open(file_name,'rb') as pkl_file:
					...
					pkl_file.close()
				# extract all computed fidelities and get the index of best real
				best_fid=[i[1] for i in all_results]
				optimal_indx=np.argmax(best_fid)
				n_sample = len(all_results)
				return n_sample, optimal_indx, all_results
			except EOFError :
				return 0, None, []
		else:
			return 0, None, []
